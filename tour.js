...
var cmds = {
	...
	tour: function (target, room, user, connection) {
		...
		// why do we need this? -> this.parse('/endpoll');
		tour.reset(rid);
		...
		Rooms.rooms[rid].addRaw('<hr /><h2><font color="green">' + tour.sanitize(user.name) + ' has started a ' + Tools.data.Formats[targets[0]].name + ' Tournament.' + joinHelp + '</h2><b><font color="blueviolet">PLAYERS:</font></b> ' + targets[1] + '<br /><font color="blue"><b>TIER:</b></font> ' + Tools.data.Formats[targets[0]].name + '<hr />');
...
Rooms.BattleRoom.prototype.win = function (winner) {
	...
	if (this.rated) {
		...
		//image leaderboard top 10 shit
		var oldp1rating = oldacre, newp1rating = acre;
						
						var reasons = ''+(acre-oldacre)+' for '+(p1score>0.99?'winning':(p1score<0.01?'losing':'tying'));
						if (reasons.substr(0,1) !== '-') reasons = '+'+reasons;
						self.addRaw(tour.sanitize(p1)+'\'s rating: '+oldacre+' &rarr; <strong>'+acre+'</strong><br />('+reasons+')');

						oldacre = Math.round(data.p2rating.oldacre);
						acre = Math.round(data.p2rating.acre);
		//image leaderboard top 10 shit
		var oldp2rating = oldacre, newp2rating = acre;
						
						reasons = ''+(acre-oldacre)+' for '+(p1score>0.99?'losing':(p1score<0.01?'winning':'tying'));
						if (reasons.substr(0,1) !== '-') reasons = '+'+reasons;
						self.addRaw(tour.sanitize(p2)+'\'s rating: '+oldacre+' &rarr; <strong>'+acre+'</strong><br />('+reasons+')');

						Users.get(p1).cacheMMR(rated.format, data.p1rating);
						Users.get(p2).cacheMMR(rated.format, data.p2rating);
						self.update();
		//image leaderboard top 10 shit
		if (toId(rated.format) == "ou") {
			if (!leaderboard[newp1rating]) leaderboard[newp1rating] = new Object();
			if (!leaderboard[newp2rating]) leaderboard[newp2rating] = new Object();
			//delete oldrating off of leaderboard
			var old1 = oldp1rating;
			var old2 = oldp2rating;
			if (leaderboard[old1]) delete leaderboard[old1][p1];
			if (leaderboard[old2]) delete leaderboard[old2][p2];
			//add new ratings
			leaderboard[newp1rating][p1] = true;
			leaderboard[newp2rating][p2] = true;
			top10(old1, old2, newp1rating, newp2rating);
		}
					} catch (e) {
						self.addRaw(e);
